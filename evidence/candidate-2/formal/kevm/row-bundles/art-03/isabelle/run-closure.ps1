...
$theoryPath = Join-Path $PSScriptRoot 'ART_03_Constructor_Resolved_Runtime_Binding.thy'
...
$buildCommand = "export PATH=/usr/local/bin:/usr/bin:/bin; cd '$repositoryCyg'; '$isabelle' build -c -o record_proofs=1 -d '$adsCyg' -d '$foundationCyg' -d '$ercTrustCyg' -d '$rowIsabelleCyg' ERC_TRUST_ART_03"
...
$exportCommand = "export PATH=/usr/local/bin:/usr/bin:/bin; '$isabelle' export -d '$adsCyg' -d '$foundationCyg' -d '$ercTrustCyg' -d '$rowIsabelleCyg' -x '*:erc-trust-art-03/proof-trust.txt' -O '$exportCyg' ERC_TRUST_ART_03"
...
if ($null -eq $proofAuditFile) { throw 'ART-03 proof-trust export is missing' }
$proofAuditText = [System.IO.File]::ReadAllText($proofAuditFile.FullName)
$qualifiedTheorem = 'ART_03_Constructor_Resolved_Runtime_Binding.constructor_resolved_local_runtime_is_hash_bound'
if (-not ($proofAuditText.Contains('status=PASS') -and $proofAuditText.Contains("qualified_theorem=$qualifiedTheorem") -and $proofAuditText.Contains('oracle_dependency_count=0'))) {
  throw "ART-03 proof-trust export failed validation: $proofAuditText"
}

$report = [ordered]@{
  status = 'PASS'; session = 'ERC_TRUST_ART_03'; theoremName = 'constructor_resolved_local_runtime_is_hash_bound'
