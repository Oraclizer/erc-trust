...
$theoryPath = Join-Path $PSScriptRoot 'ABI_03_Trailing_Calldata_Reverts_And_Stutters.thy'
...
$buildCommand = "export PATH=/usr/local/bin:/usr/bin:/bin; cd '$repositoryCyg'; '$isabelle' build -c -o record_proofs=1 -d '$adsCyg' -d '$foundationCyg' -d '$ercTrustCyg' -d '$rowIsabelleCyg' ERC_TRUST_ABI_03"
...
$exportCommand = "export PATH=/usr/local/bin:/usr/bin:/bin; '$isabelle' export -d '$adsCyg' -d '$foundationCyg' -d '$ercTrustCyg' -d '$rowIsabelleCyg' -x '*:erc-trust-abi-03/proof-trust.txt' -O '$exportCyg' ERC_TRUST_ABI_03"
...
if ($null -eq $proofAuditFile) { throw 'ABI-03 proof-trust export is missing' }
$proofAuditText = [System.IO.File]::ReadAllText($proofAuditFile.FullName)
$qualifiedTheorem = 'ABI_03_Trailing_Calldata_Reverts_And_Stutters.abi_03_trailing_calldata_reverts_and_stutters'
if (-not ($proofAuditText.Contains('status=PASS') -and $proofAuditText.Contains("qualified_theorem=$qualifiedTheorem") -and $proofAuditText.Contains('oracle_dependency_count=0'))) {
  throw "ABI-03 proof-trust export failed validation: $proofAuditText"
}

$report = [ordered]@{
  status = 'PASS'
  session = 'ERC_TRUST_ABI_03'
  theoremName = 'abi_03_trailing_calldata_reverts_and_stutters'
