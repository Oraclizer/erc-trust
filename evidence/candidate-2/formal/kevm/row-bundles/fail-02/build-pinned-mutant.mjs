#!/usr/bin/env node
// Future heavy step for FAIL-02. Do not run during the static preparation wave.
...
const canonicalInputPath = path.join(
  repositoryRoot,
  "evidence",
...
const replacement = anchor.replace(
  "        if (outcome == TrustTypes.AssessmentOutcome.REJECTED) revert TrustRejected(commandId, reason);",
  [
    "        // FAIL-02 executable mutant: misclassify dependency denial as operational failure.",
    "        if (outcome == TrustTypes.AssessmentOutcome.REJECTED) {",
    "            revert TrustOperationalFailure(commandId, reason, dependency);",
    "        }",
  ].join("\n"),
);
if (canonicalSource.split(anchor).length !== 2) throw new Error("FAIL-02 mutation anchor is not unique");
...
if (sha256(lockBytes) !== "3134e7692086170f86b6dd42e7ebd0256c188da8db8cbd17241c3d2c8d315196") {
  throw new Error("FAIL-02 actual dependency lock drift");
}
...
console.log(JSON.stringify({
  ...
  obligationId: "FAIL-02",
  compilerVersion: solc.version,
  compilerBinarySha256: solc.binarySha256,
  actualDependencyLockSha256: sha256(lockBytes),
