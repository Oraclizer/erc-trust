...
| T-N16 | ERC-7943 absolute frozen delta maps an increase to FREEZE; equal or decreasing raw calls create no typed state change and a decrease requires an explicit UNFREEZE reversal; forced transfer needs a typed transfer binding. | PROVED (abstract model) | Same-transaction context mechanism and raw-call negative vectors. |
