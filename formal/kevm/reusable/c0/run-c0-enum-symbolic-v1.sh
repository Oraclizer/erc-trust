...
: "${C0_ENUM6_CLAIM_ID:?set C0_ENUM6_CLAIM_ID}"
...
source_manifest="$repository_root/formal/kevm/reusable/c0/enum-symbolic-word-v1/source-manifest-v1.json"
...
readonly expected_source_manifest=8d33a017fdd93d2d3469e5a247b54bc20646f43e2f8160808ee55d71c590e9af
...
mapfile -t contract < <(python3 - "$source_manifest" "$definition_receipt" "$C0_ENUM6_CLAIM_ID" <<'PY'
import json, sys
with open(sys.argv[1], encoding="utf-8") as handle:
    manifest = json.load(handle)
with open(sys.argv[2], encoding="utf-8") as handle:
    receipt = json.load(handle)
claim = next((entry for entry in manifest["claims"] if entry["id"] == sys.argv[3]), None)
if claim is None:
    raise SystemExit(f"unknown enum claim: {sys.argv[3]}")
