...
[[ $timeout_seconds =~ ^[0-9]+$ ]] && (( timeout_seconds > 0 && timeout_seconds <= 1800 )) || {
  echo "C0_UINT48_TIMEOUT_SECONDS must be 1..1800" >&2
  exit 64
}
[[ ! -e $C0_UINT48_OUTPUT_ROOT ]] || { echo "output root exists: $C0_UINT48_OUTPUT_ROOT" >&2; exit 64; }

repository_root=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/../../../.." && pwd -P)
source_manifest="$repository_root/formal/kevm/reusable/c0/uint48-symbolic-word-v1/source-manifest-v1.json"
lock="$repository_root/formal/kevm/dependencies.lock.json"
readonly expected_source_manifest=a624c81bbc55f2172222a6508536cee7dd138fa1969d6a097039e2e0a5a81cbd
...
[[ -x $kprove && -x $kore_rpc ]] || { echo "pinned executable missing" >&2; exit 66; }
...
if claim is None:
    raise SystemExit(f"unknown UINT48 claim: {claim_id}")
print(claim["path"])
print(claim["module"])
print(claim["sha256"])
print(claim["role"])
...
native_root=$(mktemp -d /tmp/erc-trust-c0-uint48-canonical-XXXXXX)
...
if [[ $parse_only == true ]]; then
  ...
  setsid "$kprove" "$native_root/package.k" \
    ...
    --spec-module "$module" \
    --dry-run \
    --emit-json-spec "$C0_UINT48_OUTPUT_ROOT/parsed-spec.json" \
  ...
  printf '{"claimId":"%s","role":"%s","profile":"canonical","parseOnly":true,"backendStarted":%s,"exitCode":%s,"wallSeconds":%s,"booster":false,"assumeDefined":false,"resumed":false,"familyCredit":0}\n' \
...
setsid timeout --signal=TERM --kill-after=30s "${timeout_seconds}s" \
  kevm prove "$native_root/package.k" \
    ...
    > "$C0_UINT48_OUTPUT_ROOT/prove.log" 2>&1 &
child_pid=$!
wait "$child_pid"
exit_code=$?
set -e
...
printf '%s\n' "$elapsed" > "$C0_UINT48_OUTPUT_ROOT/wall-seconds.txt"
date -Is > "$C0_UINT48_OUTPUT_ROOT/ended-at.txt"
printf '{"claimId":"%s","role":"%s","profile":"canonical","parseOnly":false,"exitCode":%s,"wallSeconds":%s,"timeoutSeconds":%s,"booster":false,"assumeDefined":false,"workers":1,"forceSequential":true,"breakEveryStep":true,"resumed":false,"familyCredit":0}\n' \
