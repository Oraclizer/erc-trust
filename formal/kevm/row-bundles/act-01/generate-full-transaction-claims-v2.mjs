...
//
// usage:
//   node generate-full-transaction-claims-v2.mjs --state-capture-root <feasibility output root>
//
...
//              rather than left symbolic. requires still asserts every one of the 59 initially-zero footprint keys
//              (26 zero-to-nonzero + 33 unchanged-zero) absent, now discharged concretely against the empty tail;
...
if (inputIndex < 0 || !process.argv[inputIndex + 1]) {
  throw new Error("usage: node generate-full-transaction-claims-v2.mjs --state-capture-root <path>");
}
const inputRoot = resolve(process.argv[inputIndex + 1]);
const outputRoot = resolve(import.meta.dirname, "full-transaction-v2");
const generatorPath = resolve(import.meta.dirname, "generate-full-transaction-claims-v2.mjs");
...
const restAbsence = initialZeroKeys.map((key) => `notBool ${dec(key)} in_keys(.Map)`);
...
const manifest = {
  ...
  generator: {
    path: "formal/kevm/row-bundles/act-01/generate-full-transaction-claims-v2.mjs",
  ...
  storageFrame: {
    ...
    initialZeroBinding: "all 59 initially-zero footprint keys asserted absent from the empty tail, discharged concretely rather than symbolically",
  ...
  claims: [
    { path: "formal/kevm/row-bundles/act-01/full-transaction-v2/full-transaction-finalization-spec.k", sha256: finalizeSha256, module: "TRUST-ACT-01-FULL-TRANSACTION-FINALIZATION-SPEC" },
    { path: "formal/kevm/row-bundles/act-01/full-transaction-v2/full-transaction-event-order-spec.k", sha256: eventSha256, module: "TRUST-ACT-01-FULL-TRANSACTION-EVENT-ORDER-SPEC" },
  ],
  expected: {
    finalFrozenTarget: "1",
    lifecycle: "APPLIED",
