...
//
// usage:
//   node generate-full-transaction-claims-v1.mjs --state-capture-root <feasibility output root>
//
...
//   storage    <storage> ( 29 captured-nonzero entries  STORAGE_REST ) => ( 55 captured-nonzero entries  STORAGE_REST ).
//              One shared remainder STORAGE_REST carries every non-footprint slot unchanged. requires asserts every one
//              of the 59 initially-zero footprint keys (26 zero-to-nonzero + 33 unchanged-zero) absent from STORAGE_REST,
//              so they are exactly zero before the transaction; because STORAGE_REST is the same variable on both sides,
...
//   accounts   Four named accounts (coinbase 0, sender, dependency, token) plus one shared remainder ACCOUNTS_REST. A K
...
if (inputIndex < 0 || !process.argv[inputIndex + 1]) {
  throw new Error("usage: node generate-full-transaction-claims-v1.mjs --state-capture-root <path>");
}
const inputRoot = resolve(process.argv[inputIndex + 1]);
const outputRoot = resolve(import.meta.dirname, "full-transaction-v1");
const generatorPath = resolve(import.meta.dirname, "generate-full-transaction-claims-v1.mjs");
...
function renderStorageSide(nonzeroKeys, depth) {
  ...
  lines.push(`${indent(depth + 2)}STORAGE_REST`);
  return `${indent(depth)}(\n${lines.join("\n")}\n${indent(depth)})`;
}
const preStorageSide = renderStorageSide(preNonzeroKeys, 16);
const postStorageSide = renderStorageSide(postNonzeroKeys, 16);
const restAbsence = initialZeroKeys.map((key) => `notBool ${dec(key)} in_keys(STORAGE_REST)`);
const requiresClause = restAbsence.join("\n      andBool ");
...
const manifest = {
  ...
  obligationId: "ACT-01",
  frameVersion: "v3-fresh-audit-minimum-contract-compact",
  generator: {
    path: "formal/kevm/row-bundles/act-01/generate-full-transaction-claims-v1.mjs",
  ...
  claimIdentity: {
    ...
    normalized: {
      ...
      accountRemainder: "ACCOUNTS_REST (shared, structurally disjoint, unchanged)",
    },
  ...
  storageFrame: {
    ...
    restAbsenceAtomCount: restAbsence.length,
    remainderVariable: "STORAGE_REST (same variable on both sides; every non-footprint slot preserved unchanged)",
    initialZeroBinding: "all 59 initially-zero footprint keys asserted absent from STORAGE_REST (exactly zero before the transaction)",
    finalZeroBinding: "the 33 final-zero keys stay absent because STORAGE_REST is unchanged; the 26 changed keys reappear as concrete post entries",
    origStorage: "don't-care (write-only under #finalizeStorage; unread while useGas=false)",
    changedInitialZeroKeySet,
  },
  accountFrame: {
    encoding: "FOUR_NAMED_ACCOUNTS_PLUS_SHARED_ACCOUNTS_REST",
  ...
  claims: [
    { path: "formal/kevm/row-bundles/act-01/full-transaction-v1/full-transaction-finalization-spec.k", sha256: finalizeSha256, module: "TRUST-ACT-01-FULL-TRANSACTION-FINALIZATION-SPEC" },
    { path: "formal/kevm/row-bundles/act-01/full-transaction-v1/full-transaction-event-order-spec.k", sha256: eventSha256, module: "TRUST-ACT-01-FULL-TRANSACTION-EVENT-ORDER-SPEC" },
  ],
  expected: {
    finalFrozenTarget: "1",
    lifecycle: "APPLIED",
    ...
    finalizationClaimRestAbsenceOccurrences: countMatches(finalizeClaim, /in_keys\(STORAGE_REST\)/g),
