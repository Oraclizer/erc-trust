...
$theoryPath = Join-Path $PSScriptRoot 'SEP_04_Receipt_Event_Binding.thy'
...
$buildCommand = "export PATH=/usr/local/bin:/usr/bin:/bin; cd '$repositoryCyg'; '$isabelle' build -c -o record_proofs=1 -d '$adsCyg' -d '$foundationCyg' -d '$ercTrustCyg' -d '$rowIsabelleCyg' ERC_TRUST_SEP_04"
...
$exportCommand = "export PATH=/usr/local/bin:/usr/bin:/bin; '$isabelle' export -d '$adsCyg' -d '$foundationCyg' -d '$ercTrustCyg' -d '$rowIsabelleCyg' -x '*:erc-trust-sep-04/proof-trust.txt' -O '$exportCyg' ERC_TRUST_SEP_04"
...
if ($null -eq $proofAuditFile) { throw 'SEP-04 proof-trust export is missing' }
$proofAuditText = [System.IO.File]::ReadAllText($proofAuditFile.FullName)
$qualifiedTheorem = 'SEP_04_Receipt_Event_Binding.receipt_preimage_matches_storage_return_and_final_event'
if (-not ($proofAuditText.Contains('status=PASS') -and $proofAuditText.Contains("qualified_theorem=$qualifiedTheorem") -and $proofAuditText.Contains('oracle_dependency_count=0'))) {
  throw "SEP-04 proof-trust export failed validation: $proofAuditText"
}

$report = [ordered]@{
  status = 'PASS'
  session = 'ERC_TRUST_SEP_04'
  theoremName = 'receipt_preimage_matches_storage_return_and_final_event'
