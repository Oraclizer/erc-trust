...
import {ERC3643HookDeployment} from "./ERC3643HookDeployment.sol";
...
/// @notice Profile-specific surface of the fresh ERC-3643 hook endpoint. Its ERC-165 identifier
///         is separate from the kernel identifier; the kernel interface is unchanged.
interface IERC3643HookProfile {
    ...
    /// @notice True only while the constructor seal, sole Agent path, callback, and dependency code remain live.
...
/// @notice Kernel version 2 endpoint of a fresh T-REX unit with same-transaction frozen-target enforcement.
...
contract ERC3643HookAdapter is IERCTrustKernel, IERC3643HookProfile {
    ...
    /// @dev No import entry is accepted by the fresh profile; this domain separates unreachable import helpers.
    ...
    constructor(
        ...
        address profileGovernor_ = ERC3643HookDeployment.prepare(tokenCreationSource, holder, supply, eligible);
        if (authority_ == address(0) || authorityRef_ == bytes32(0)) {
        ...
        ERC3643HookGovernor(profileGovernor_).sealFresh();
        _activateEmptySeal();
    }
    ...
    /// @dev Empty-state activation used only by the fresh endpoint constructor.
    function _activateEmptySeal() internal {
        ERC3643ProfileTypes.ImportEntry[] memory entries = new ERC3643ProfileTypes.ImportEntry[](0);
        if (_sealed || !profileGovernor.topologySealed() || profileGovernor.exclusiveAdapter() != address(this)) {
            revert TrustOperationalFailure(bytes32(0), REASON_SEAL_INVALID, _tokenRef());
        }
        bytes32 binding = profileGovernor.sealedBinding();
    ...
    function supportsInterface(bytes4 interfaceId) external pure returns (bool) {
        return interfaceId != 0xffffffff
            && (interfaceId == ERC165_INTERFACE_ID
                ...
                || interfaceId == type(IERC3643HookProfile).interfaceId);
    }
    ...
    /// @dev Full is computed from the live constructor-sealed topology; no existing-token import is accepted.
    ...
    /// @dev The balance callback normally keeps the target materialised. This permissionless path is
    ///      retained as an idempotent diagnosis and repair entrypoint under the same live topology checks.
    ...
    /// @notice Materialises both owned frozen targets before the bound T-REX balance call can return.
    /// @dev The pinned token's freeze and unfreeze functions have no Compliance callback, so this path
    ///      does not recurse. It intentionally may run inside a canonical regulatory command.
    function onTokenBalanceChanged(address from, address to) external {
        ...
        _requireLiveTopology(bytes32(0));
        if (from != address(0)) _syncFrozen(bytes32(0), from);
    ...
    function _profileSchema() internal pure returns (bytes32) {
        return TrustKernelTypes.PROFILE_ERC3643_VERIFIED_FULL;
    }
    ...
    function _bind(TrustKernelTypes.BindingKind kind, address dependency, bytes32 sealedBinding) internal {
        ...
        _bindings[kind] = ERC3643ProfileTypes.DependencyBinding({
            ...
            bindingHash: keccak256(
                abi.encode(
                    TrustKernelTypes.DOMAIN, uint8(kind), dependency, code, sealedBinding, _profileSchema(), SEAL_EPOCH
                )
            )
        });
    }
